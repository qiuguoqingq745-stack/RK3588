...
int MppDecoder::Decode(uint8_t *pkt_data, int pkt_size, int pkt_eos)
{
    ...
    RK_U32 pkt_done = 0;//数据有没有成功送进解码器
    RK_U32 err_info = 0;
    MPP_RET ret = MPP_OK;
    MppCtx ctx = data->ctx;//MPP 解码上下文和接口
    MppApi *mpi = data->mpi;//MPP 解码上下文和接口
    int got_frames = 0;//这次总共有没有拿到帧
    if (packet == NULL)  
    {
    ...
    do
    {
        ...
        try_again:
            ...
            {
                // 如果解码器还没吐出帧（比如只是送入了 SPS/PPS，或者 B 帧乱序），
                // 不要死等！直接 break 回去让外层循环继续喂数据，否则会导致 RTSP 积压丢包
                break;
            }
            if (MPP_OK != ret)
            {
            ...
            if (frame)
            {
                ...
                else
                {
                    ...
                    if (callback != nullptr)
                    {
                        ...
                        // // 真正的零拷贝：只需要 fd，不再映射虚拟地址
                        // int fd = mpp_buffer_get_fd(mpp_frame_get_buffer(frame));
                        // callback(this->userdata, hor_stride, ver_stride, hor_width, ver_height, format, fd, nullptr, this->id);
                        MppFrameFormat format = mpp_frame_get_fmt(frame);
