...
int MppEncoder::EncodeFrame(uint8_t* bgr_data, int width, int height,
    ...
    rga_buffer_t dst_buf = wrapbuffer_virtualaddr_t((void*)yuv_buffer_, width, height, width, height, RK_FORMAT_YCbCr_420_SP);
    ...
    if (status != IM_STATUS_SUCCESS) {
        ...
        // OpenCV outputs I420 (YYYY UUUU VVVV), but we need NV12 (YYYY UVUVUV)
        int y_size = width * height;
        int uv_size = y_size / 4;
        memcpy(yuv_buffer_, yuv.data, y_size);
        
        uint8_t* u_ptr = yuv.data + y_size;
        uint8_t* v_ptr = u_ptr + uv_size;
        uint8_t* uv_out = yuv_buffer_ + y_size;
