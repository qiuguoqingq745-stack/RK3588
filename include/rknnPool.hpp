...
int rknn_lite::interf(detect_result_group_t &detect_result_group)
{
    ...
    // 优先使用 RGA 硬件加速进行 BGR->RGB 转换及缩放，大幅降低 CPU 占用
    rga_buffer_t src_buf = wrapbuffer_virtualaddr(ori_img.data, img_width, img_height, RK_FORMAT_BGR_888);
    rga_buffer_t dst_buf = wrapbuffer_virtualaddr(img.data, width, height, RK_FORMAT_RGB_888);

    IM_STATUS status = imresize(src_buf, dst_buf);
    if (status != IM_STATUS_SUCCESS) {
        // RGA 硬件加速失败时回退到 OpenCV
    ...
    const float nms_threshold = NMS_THRESH;//去掉重叠过高的重复框
    const float box_conf_threshold = BOX_THRESH;//用于过滤低置信度框
    ...
    // Draw Objects (已移至 main.cpp 的 fusion_manager，此处无需画框)
    // char text[256];
